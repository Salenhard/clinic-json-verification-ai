...
Верни ТОЛЬКО валидный JSON (без markdown):
{
  ...
  "supplement_json": {
    "ПРАВИЛА": "Только поля для ДОБАВЛЕНИЯ или ИСПРАВЛЕНИЯ. Структура = структура исходного JSON. Только null/отсутствующие/некорректные поля. Значения строго из рекомендаций."
  },
  "overall_comment": "..."
}
"""
    ...
    def _build_prompt(self, chunk_text: str, chunk_index: int, total_chunks: int, json_data: str) -> str:
        # Формируем промпт с правильными фигурными скобками
        prompt = (
            "Задача: проверить, насколько JSON-документ соответствует клиническим рекомендациям,\n"
            "и сформировать патч для его дополнения.\n\n"
            f"JSON-ДОКУМЕНТ:\n{json_data}\n\n"
            f"ФРАГМЕНТ КЛИНИЧЕСКИХ РЕКОМЕНДАЦИЙ (часть {chunk_index + 1} из {total_chunks}):\n{chunk_text}\n\n"
            "Проверь:\n"
            "1. Полноту (все ли поля, которые должны быть по рекомендациям, присутствуют)\n"
            "2. Корректность значений (соответствуют ли они рекомендациям)\n"
            "3. Отсутствующие критически важные данные\n"
            ...
            '  ],\n'
            '  "missing_fields": ["поле1", "поле2"],\n'
            '  "suggestions": {"поле": "рекомендуемое значение"},\n'
            '  "supplement_json": {\n'
            '    "<ключ из исходного JSON>": "<значение согласно рекомендациям>"\n'
            '    // ТОЛЬКО поля для ДОБАВЛЕНИЯ или ИСПРАВЛЕНИЯ.\n'
            '    // Структура должна точно соответствовать структуре исходного JSON.\n'
            '    // Включай только null/отсутствующие/некорректные поля.\n'
            '    // Значения строго из клинических рекомендаций.\n'
            '  },\n'
            '  "overall_comment": "..."\n'
            '}'
        )
